...
// Default Groq model – can be changed via .env.local
// GROQ_MODEL_ID is optional; if not set, we use a good default.
export const DEFAULT_MODEL_ID: string =
  (process.env.GROQ_MODEL_ID as string | undefined) ||
  "llama-3.3-70b-versatile";

/**
 * Builds the prompt for resume vs job description analysis.
 * The model is instructed to return ONLY valid JSON
 * with keys: overallSummary, keyStrengths, improvementSuggestions.
 */
export const prompt = (resumeText: string, jdText: string): string => `
You are an ATS-style resume and job description analyzer.

Task:
- Compare this RESUME with this JOB DESCRIPTION.
- Analyze skills match, experience relevance, and gaps.
- Then respond ONLY with valid JSON, nothing else.

Output JSON format (exact keys):

{
  "overallSummary": "short paragraph explaining how well the resume matches the JD",
  "keyStrengths": [
    "point 1 about strengths",
    "point 2 about strengths"
  ],
  "improvementSuggestions": [
    {
      "area": "Section or topic to improve",
      "suggestion": "Concrete suggestion about what to change/add",
      "example": "Example bullet/phrasing or idea to add in resume"
    }
  ]
}
